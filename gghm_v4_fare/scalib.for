...
c
      character*80   header
      integer*2      iter,mkt
      integer*4      nseg,ci,c,t,t1,t2,origsta
      real*8         values(7),pobs,pest,econ,ncon,rcon
      real*8         tesum(38,7),oesum(38,7),dfsum(38,7)
      real*8         maxval,minval,incdat(6),oshare
      real*8         otrip(25,6),ttrip(25,6),ptrip(25,6)
      real*8         tottrn(31,31,6),gorail(21,21)
      real*8         ttcsubway(21,21),stasum(1000,12)
      real*8         nonmot(31,31,3),stasum2(max_stations,7)
      logical        exists
      data           minval/-15.0/
c
      oesum=0.0
      dfsum=0.0
      otrip=0.0
      open(141,file='ptrip.csv',status='unknown',form='formatted')
      do k=1,6
      do k1=1,25
      k2=k1*5
      write(141,200) k,k2,ptrip(k1,k)
  200 format(i2,',',i3,',',f12.2)
      end do
      end do
c
...
C
 ...
 4763 FORMAT(/15X,'GO Rail Union Station Walk Constant',/,
     ...
     *  1X,' Level      Value   Value   Constant Adjustment  Constant'/
     *  1X,'---------  ------- ------- --------- ---------- ---------')
c        IF(FINCH.LE.0) THEN
c        WRITE(26,4764)
c        WRITE(*,4764)
c4764    FORMAT(/' FINCH STATION VALUE UNDEFINED')
c        STOP
c        END IF
