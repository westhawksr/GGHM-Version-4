...
C--------------------------------------------------------------------
      SUBROUTINE STACAP(ITER,stasum,converg)
...
C
      CONVERG=.TRUE.
      IF(BPFACE.LE.0.OR.BUSPNROCC.LE.0) THEN
      WRITE(26,8005) BPFACE,BUSPNROCC
      WRITE(*,8005) BPFACE,BUSPNROCC
 8005 FORMAT(/' STACAP 8005 (F) EITHER BPFACE (',F5.2,') OR',
     *       ' BUSPNROCC (',F5.2,') ARE ZERO'/)
      STOP 8005
      END IF
...
C
C     WRITE(26,9001) CONVERG,ITER,CITER
 9001 FORMAT(/' STATION CAPACITY RESTRAINT PARAMETER SUMMARY'/
     *        ' CONVERGE=',2X,L1/
     *        ' ITER    =',I3/
     *        ' CITER   =',I3/)
      IF(CONVERG.OR.(ITER.EQ.CITER)) THEN
      OPEN(10,FILE=FSTAOUT,FORM='FORMATTED',STATUS='UNKNOWN')
      OPEN(8,FILE='STAVCR.CSV',FORM='FORMATTED',STATUS='UNKNOWN')
...
C..SPACES USED & SHADOW PRICE
       ...
       IUSED=IFIX(USED)
       IF(IUSED.GT.0) THEN
       WRITE(10,8335) IEQUIV(NSTA),IUSED,STADATA(NS,5)
 8335  FORMAT(I4,',',I8,',',F5.0)
       WRITE(8,8336) IEQUIV(NSTA),(STAVCR(NS,K),K=1,ITER)
