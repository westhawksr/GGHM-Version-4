...
C
      ...
      REAL*4       UTIL,ADIST(max_stations),ATIME(max_stations),CDIST,
     *             STAUTL,STAZNE(6,MAX_STATIONS,MAX_IZONES)
...
C
      IC=CSTA-MAX_IZONES
	    DAR=0.0
	    IF(ADIST(ic).GT.0) DAR=(ADIST(ic)/hwydst(JZ))-0.5
	    DAR=AMAX1(DAR,0.0)
      UTIL=COEFF(16)*ATIME(ic)+opcost*COEFF(16)*ADIST(ic)+
...
C
 ...
 9025 FORMAT(1X,'DESTINATION STATION --> EGRESS ZONE COMPUTATIONS'/
     ...
     *       1X,'UTILITY VALUE            =',F10.5/)
      ...
      WRITE(26,9047) NAME,CSTA,STANAME(IC),ATIME(ic),
