...
c
      ...
      if(sc.lt.0) return
      ic=iequiv(sta)
      if(sdetail) write(26,7003) ic,staname(sc),name(imode)
 7003 format(/' ORIGIN STATION=',i5,1x,a37,1x,a15/)
c
...
C     if(sdetail) write(26,7007) iequiv(equiv3(sc2,1)),
C    *               staname(equiv3(sc2,1)-max_izones),
C    *               iequiv(equiv3(sc2,2)),
C    *               staname(equiv3(sc2,2)-max_izones)
C7007 format(' equiv3(1)=',i5,1x,a37,1x,
C    *       ' equiv3(2)=',i5,1x,a37)
c
...
c
 ...
 7002 format(' match for CR station=',i5,1x,a37,
...
c
      ...
      dc=iequiv(matr+max_izones)
      if(dc.eq.gounion) ktxfer=0.0
      if(stazne(2,sc4,jz).ne.0.0.and.(.not.trnegr)) then
     ...
     *          ktxfer
      ...
      if(staegr(3,sc4,jz).ne.0.0.and.trnegr) then
      util=stasta(2,sc,sc2)+stasta(2,sc3,sc4)+staegr(3,sc4,jz)+
     *          coeff(7)*(stadata(sc,9)+stadata(sc2,9)) +
     *          coeff(7)*(stadata(sc3,9)+stadata(sc4,9)) +
     *          ktxfer
      end if
c
      if(FOURDETAIL.and.(.not.trnegr)) 
     ...
     *               ktxfer,util,
     *               staind(sc4,jz),mutil
      if(FOURDETAIL.and.trnegr)      
     *               write(26,7008) name(imode),ic,staname(sc),
     *               iequiv(sc2+max_izones),staname(sc2),
     *               iequiv(sc3+max_izones),staname(sc3),
     *               iequiv(sc4+max_izones),staname(sc4),
     *               stadata(sc,9),stadata(sc2,9),
     *               stadata(sc3,9),stadata(sc4,9),
     *               stasta(2,sc,sc2),
     *               stasta(2,sc3,sc4),
 ...
 7001 format(/' 4 PART PATH CONSIDERATION FOR ',A15,/
     ...
     *               staegr(3,mdst,jz),
     *               ktxfer,mutil,mutil
 7008 format(/' 4 PART PATH CONSIDERATION FOR ',A15,/
     *       ' ------------------------------'/
     *       ' ORIGIN STATION                       =',I5,1X,A37/
     *       ' DESTINATION STATION                  =',I5,1X,A37/
     *       ' URBAN RAIL ORIGIN STATION            =',I5,1X,A37/
